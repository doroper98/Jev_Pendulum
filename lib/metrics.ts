...
export function summarize(recording: Recording) {
  ...
  return { config: { ...recording.config }, elapsed: last.time, bestBalance, success: bestBalance >= 2, maxAngle: Math.max(...frames.flatMap(f => f.state.slice(1, recording.config.topology === 'single' ? 2 : 3).map(a => Math.abs(degrees(a))))), travel, switches, calls: decisions.reduce((s, d) => s + (d.calls || 0), 0), inputTokens: tokenKnown ? decisions.reduce((s, d) => s + (d.inputTokens || 0), 0) : null, meanLatency: decisions.length ? decisions.reduce((s, d) => s + d.latency, 0) / decisions.length : 0, steps: decisions.length };
}
