...
function run(controller, clock, hz, seconds) {
  const e = new Experiment({ ...DEFAULT_CONFIG, controller, clock, duration: 120 });
  let notifications = 0;
  e.subscribe(() => notifications++);
  e.start(); const start = e.lastFrame;
  ...
  assert.ok(notifications <= seconds * 10 + 2, `too many UI updates: ${notifications}`);
  const recording = e.export(); e.dispose(); return recording;
}

for (const controller of ['local', 'manual', 'random']) {
  test(`${controller}: both clocks run at 1x with identical physics at 30/60/120/144Hz`, () => {
    const seconds = controller === 'local' ? 10 : 1;
    const baseline = run(controller, 'wait', 60, seconds);
    for (const clock of ['wait', 'realtime']) for (const hz of [30, 60, 120, 144]) {
      const actual = run(controller, clock, hz, seconds);
...
test('irregular display frames preserve all elapsed time and physics steps', () => {
  const baseline = run('local', 'wait', 60, 10);
  const e = new Experiment({ ...DEFAULT_CONFIG, controller: 'local', duration: 120 });
  e.start(); const start = e.lastFrame; let elapsed = 0, i = 0;
...
test('display interpolation stays between recorded frames and never changes physics', () => {
  const e = new Experiment({ ...DEFAULT_CONFIG, controller: 'local' });
  e.start(); e.tick(e.lastFrame + 30);
...
test('explicit local response delay is still honored in wait mode', async () => {
  const e = new Experiment({ ...DEFAULT_CONFIG, controller: 'local', delay: 30 });
  const start = [...e.state], promise = e.singleStep();
  assert.equal(e.pending, true); assert.equal(e.time, 0); assert.deepEqual(e.state, start);
